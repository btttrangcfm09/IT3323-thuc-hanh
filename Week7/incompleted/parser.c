...
ConstantValue* compileConstant(void) {
  ...
  case SB_PLUS:
    ...
    // Check that constant with +/- operator must be integer type
...
void compileAssignSt(void) {
  ...
  // Check type consistency: LValue and Expression must have same basic type
...
void compileForSt(void) {
  ...
  // Check type consistency: variable, exp1, exp2 must have same basic type
...
void compileArgument(Object* param) {
  ...
  // If the parameter is a reference parameter, the argument must be an LValue
  ...
  // Check type consistency between formal parameter and actual parameter
...
void compileCondition(void) {
  ...
  type1 = compileExpression();
  // Check that expressions in condition must have basic type
  ...
  // Check type consistency: exp1 and exp2 must have same basic type
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    // Check that index expression must be integer type
    checkIntType(indexType);
    eat(SB_RSEL);
    
    // If the element is also an array, continue indexing
